"use client";

import { useState } from "react";
import { Package, MapPin, RotateCcw, Tag } from "lucide-react";
import { useLanguage } from "@/lib/i18n/language-context";
import { MotionDiv } from "@/components/shared/Motion";
import { cardHover, fadeUp, staggerContainer, tapScale } from "@/lib/animations";

export default function AccountPage() {
  const { t } = useLanguage();
  const [notice, setNotice] = useState("");

  const handleSignIn = (e: React.FormEvent) => {
    e.preventDefault();
    setNotice(t("account.loginSoon"));
  };

  const benefits = [
    {
      title: t("account.trackOrders"),
      description: t("account.trackOrdersDesc"),
      icon: Package,
      color: "bg-brand-primary-light/50 text-brand-primary"
    },
    {
      title: t("account.saveAddresses"),
      description: t("account.saveAddressesDesc"),
      icon: MapPin,
      color: "bg-[#f4e2a9]/50 text-[#6b5e31]"
    },
    {
      title: t("account.reorder"),
      description: t("account.reorderDesc"),
      icon: RotateCcw,
      color: "bg-[#ffeee8] text-[#ba1a1a]"
    },
    {
      title: t("account.specialOffers"),
      description: t("account.specialOffersDesc"),
      icon: Tag,
      color: "bg-[#e5f6fd] text-[#0288d1]"
    }
  ];

  return (
    <div className="flex-1 container-max mx-auto px-4 md:px-6 py-12 md:py-16 w-full overflow-x-hidden">
      <MotionDiv className="text-center md:text-left mb-10 md:mb-16" variants={fadeUp}>
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-brand-text mb-4">
          {t("account.title")}
        </h1>
        <p className="text-lg text-brand-text-muted max-w-2xl">
          {t("account.subtitle")}
        </p>
      </MotionDiv>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        
        {/* Left Side: Benefits */}
        <MotionDiv className="order-2 lg:order-1 space-y-6" variants={fadeUp}>
          <h2 className="text-xl font-bold text-brand-text mb-6">
            {t("account.whyCreate")}
          </h2>
          <MotionDiv className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4" variants={staggerContainer}>
            {benefits.map((benefit, idx) => (
              <MotionDiv key={idx} variants={fadeUp} whileHover={cardHover} className="flex items-start gap-4 p-5 bg-white border border-brand-outline rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                <div className={`p-3 rounded-xl flex-shrink-0 ${benefit.color}`}>
                  <benefit.icon className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="font-bold text-brand-text mb-1">{benefit.title}</h3>
                  <p className="text-sm text-brand-text-muted leading-relaxed">
                    {benefit.description}
                  </p>
                </div>
              </MotionDiv>
            ))}
          </MotionDiv>
        </MotionDiv>

        {/* Right Side: Sign In Card */}
        <MotionDiv className="order-1 lg:order-2" variants={fadeUp}>
          <div className="bg-white border border-brand-outline rounded-3xl p-6 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <h2 className="text-2xl font-bold text-brand-text mb-8 text-center">
              {t("account.welcomeBack")}
            </h2>
            
            <form onSubmit={handleSignIn} className="space-y-5">
              <div className="space-y-1.5">
                <label htmlFor="account-email" className="text-sm font-semibold text-brand-text">
                  {t("account.emailOrPhone")}
                </label>
                <input 
                  id="account-email"
                  type="text" 
                  placeholder={t("account.emailPlaceholder")}
                  className="w-full h-14 bg-brand-surface border border-brand-outline rounded-xl px-4 focus:outline-none focus:ring-2 focus:ring-brand-primary transition-shadow"
                  required
                />
              </div>
              
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  <label htmlFor="account-password" className="text-sm font-semibold text-brand-text">
                    {t("account.password")}
                  </label>
                  <button
                    type="button"
                    onClick={() => setNotice(t("account.loginSoon"))}
                    className="text-xs font-medium text-brand-primary hover:underline cursor-pointer"
                  >
                    {t("account.forgotPassword")}
                  </button>
                </div>
                <input 
                  id="account-password"
                  type="password" 
                  placeholder={t("account.passwordPlaceholder")}
                  className="w-full h-14 bg-brand-surface border border-brand-outline rounded-xl px-4 focus:outline-none focus:ring-2 focus:ring-brand-primary transition-shadow"
                  required
                />
              </div>

              <MotionDiv whileTap={tapScale}>
              <button 
                type="submit"
                className="w-full h-14 bg-brand-primary text-white font-semibold rounded-xl text-lg hover:bg-brand-primary/90 transition-colors shadow-md mt-4"
              >
                {t("account.signIn")}
              </button>
              </MotionDiv>
            </form>

            <div className="relative my-8">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-brand-outline"></div>
              </div>
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-white text-brand-text-muted">{t("account.orContinueWith")}</span>
              </div>
            </div>

            <MotionDiv whileTap={tapScale}>
            <button 
              type="button"
              className="w-full h-14 bg-white border-2 border-brand-outline text-brand-text font-semibold rounded-xl flex items-center justify-center gap-3 hover:bg-brand-surface transition-colors"
              onClick={() => setNotice(t("account.loginSoon"))}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
              {t("account.google")}
            </button>
            </MotionDiv>

            <div className="mt-8 text-center">
              <span className="text-brand-text-muted text-sm">{t("account.noAccount")} </span>
              <button
                type="button"
                onClick={() => setNotice(t("account.loginSoon"))}
                className="text-brand-primary font-semibold text-sm hover:underline cursor-pointer"
              >
                {t("account.createAccount")}
              </button>
            </div>
            {notice && (
              <p className="mt-5 rounded-xl bg-brand-primary-light/40 px-4 py-3 text-center text-sm font-medium text-brand-text-muted" role="status">
                {notice}
              </p>
            )}
          </div>
        </MotionDiv>
      </div>
    </div>
  );
}
